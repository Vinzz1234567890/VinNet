...
(
    ...
    sleep 10
    [ -f /proc/sys/net/core/netdev_max_backlog ] && echo "300" >/proc/sys/net/core/netdev_max_backlog                                   # Default netdev_max_backlog=1000
    [ -f /proc/sys/net/core/rmem_default ] && echo "262144" >/proc/sys/net/core/rmem_default                                            # Default rmem_default=212992
    [ -f /proc/sys/net/core/rmem_max ] && echo "4194304" >/proc/sys/net/core/rmem_max                                                   # Default rmem_max=16777216
    [ -f /proc/sys/net/core/wmem_default ] && echo "262144" >/proc/sys/net/core/wmem_default                                            # Default wmem_default=212992
    [ -f /proc/sys/net/core/wmem_max ] && echo "4194304" >/proc/sys/net/core/wmem_max                                                   # Default wmem_max=8388608
    [ -f /proc/sys/net/core/somaxconn ] && echo "1024" >/proc/sys/net/core/somaxconn                                                    # Default somaxconn=128
    ...
    [ -f /proc/sys/net/ipv4/tcp_syn_retries ] && echo "2" >/proc/sys/net/ipv4/tcp_syn_retries                                           # Default tcp_syn_retries=4
    [ -f /proc/sys/net/ipv4/tcp_synack_retries ] && echo "2" >/proc/sys/net/ipv4/tcp_synack_retries                                     # Default tcp_synack_retries=3
    [ -f /proc/sys/net/ipv4/tcp_retries2 ] && echo "3" >/proc/sys/net/ipv4/tcp_retries2                                                 # Default tcp_retries2=8
    [ -f /proc/sys/net/ipv4/tcp_rmem ] && echo "4096 262144 4194304" >/proc/sys/net/ipv4/tcp_rmem                                       # Default tcp_rmem=524288 1048576 2097152
    [ -f /proc/sys/net/ipv4/tcp_wmem ] && echo "4096 262144 4194304" >/proc/sys/net/ipv4/tcp_wmem                                       # Default tcp_wmem=262144 212992 8388608
    [ -f /proc/sys/net/ipv4/udp_rmem_min ] && echo "8192" >/proc/sys/net/ipv4/udp_rmem_min                                              # Default udp_rmem_min=4096
    [ -f /proc/sys/net/ipv4/udp_wmem_min ] && echo "8192" >/proc/sys/net/ipv4/udp_wmem_min                                              # Default udp_wmem_min=4096
    [ -f /proc/sys/net/ipv4/tcp_tw_reuse ] && echo "2" >/proc/sys/net/ipv4/tcp_tw_reuse                                                 # Default tcp_tw_reuse=2
    [ -f /proc/sys/net/ipv4/tcp_max_syn_backlog ] && echo "1024" >/proc/sys/net/ipv4/tcp_max_syn_backlog                                # Default tcp_max_syn_backlog=128
    [ -f /proc/sys/net/ipv4/tcp_fastopen ] && echo "3" >/proc/sys/net/ipv4/tcp_fastopen                                                 # Default tcp_fastopen=1
    [ -f /proc/sys/net/ipv4/neigh/default/delay_first_probe_time ] && echo "1" >/proc/sys/net/ipv4/neigh/default/delay_first_probe_time # Default delay_first_probe_time=5
    [ -f /sys/class/net/wlan0/tx_queue_len ] && echo "1024" >/sys/class/net/wlan0/tx_queue_len                                          # Default tx_queue_len=3000

    command -v cmd && cmd wifi remove-all-suggestions
    ...
    if [ "$(getprop ro.product.device)" = "fog" ]; then
        if grep -q "VinNet" "/vendor/etc/wifi/WCNSS_qcom_cfg.ini"; then
            su shell -c "cmd notification post -S bigtext -t 'VinNet' 'MountSuccess' 'Vendor configuration has been successfully mounted'"
        else
            su shell -c "cmd notification post -S bigtext -t 'VinNet' 'MountFailed' 'Vendor configuration failed to mount. Install meta module to resolve issue'"
        fi
    fi
